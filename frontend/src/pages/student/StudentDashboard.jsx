...
import { userService, appointmentService, notificationService } from '../../api';
import {
  PageContainer,
  PageHeader,
  Card,
  ...
  useEffect(() => {
    const loadNotifications = async (showLoading = true) => {
      try {
        ...
        // Keep quiet to avoid noise on the dashboard.
      } finally {
  ...
  return (
    <PageContainer>
      ...
      <div className="grid lg:grid-cols-2 gap-8">
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-text-primary-light dark:text-text-primary">Recent Events</h2>
          </div>
          ...
          {notificationsLoading ? (
            ...
            <Card className="text-center py-8">
              <p className="text-text-secondary-light dark:text-text-secondary">No events yet</p>
            </Card>
          ) : (
            <div className="space-y-3">
              {notifications.map((event) => (
                <Card key={event.id}>
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <p className="font-medium text-text-primary-light dark:text-text-primary">
                        {event.eventName}
                      </p>
                      <p className="text-sm text-text-secondary-light dark:text-text-secondary">
                    ...
                    <p className="text-xs text-text-muted-light dark:text-text-muted">
                      {dayjs(event.eventTimestamp).format('MMM D, h:mm A')}
                    </p>
                  </div>
                </Card>
              ))}
